...
class DCVTD_DQN(base.DCVTD_ValueNet):
    ...
    def train(self):
        ...
        for i in range(batch_num):
            ...
            intrinsic_rewards = self.calc_intrinsic_reward(obs=obs, feature=feats, acts=actions)
            target_q = self.calc_target_q(obs=obs_next, feature=feat_next, rewards=rewards, intrinsic_rewards=intrinsic_rewards, dones=dones)
            dcv_target_q = self.calc_dcv_target_q(obs=obs_next, feature=feat_next, rewards=rewards, intrinsic_rewards=intrinsic_rewards, dones=dones)
            loss, q = super().train(state=[obs, feats], target_q=target_q, dcv_target_q=dcv_target_q, acts=actions, masks=masks)

            self.update()

            if i % 50 == 0:
                print('[*] DCVTD-Q LOSS:', loss, '/ Q:', q)

    def save(self, dir_path, step=0):
        model_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, self.name_scope)
        saver = tf.train.Saver(model_vars)

        file_path = os.path.join(dir_path, "DCVTD_dqn_{}".format(step))
