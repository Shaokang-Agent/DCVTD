...
class DCVTD_ActorCritic:
    def __init__(self, sess, name, handle, env, value_coef=0.1, ent_coef=0.08, gamma=0.95, batch_size=64,
                 learning_rate=1e-4):
        self.sess = sess
        self.env = env
    ...
    def _create_network(self, view_space, feature_space):
        ...
        other_input_view = input_view
        other_input_feature = input_feature
        ...
        # DCV
        DCV_flatten_view = tf.reshape(other_input_view, [-1, np.prod([v.value for v in input_view.shape[1:]])])
        DCV_h_view = tf.layers.dense(DCV_flatten_view, units=hidden_size[0], activation=tf.nn.relu)
        DCV_h_emb = tf.layers.dense(other_input_feature, units=hidden_size[0], activation=tf.nn.relu)
        DCV_dense = tf.concat([DCV_h_view, DCV_h_emb, DCV_h_view, DCV_h_emb], axis=1)
        DCV_dense = tf.layers.dense(DCV_dense, units=hidden_size[0] * 2, activation=tf.nn.relu)
        DCV_value = tf.layers.dense(DCV_dense, units=1)
        DCV_value = tf.reshape(DCV_value, (-1,))

        intrinsic_reward = tf.reduce_mean(value - DCV_value, axis=0)
        intrinsic_reward = tf.clip_by_value(0.5 * intrinsic_reward, clip_value_min=-0.15, clip_value_max=0.15)

        action_mask = tf.one_hot(action, self.num_actions)
        advantage = tf.stop_gradient(reward + intrinsic_reward - value)
        ...
        DCV_vf_loss = self.value_coef * tf.reduce_mean(tf.square(reward - DCV_value))
        neg_entropy = self.ent_coef * tf.reduce_mean(tf.reduce_sum(policy * log_policy, axis=1))
        total_loss = pg_loss + vf_loss + neg_entropy + DCV_vf_loss
        ...
        self.policy, self.value, self.DCV_value = policy, value, DCV_value
        self.train_op = train_op
        self.pg_loss, self.vf_loss, self.reg_loss, self.DCV_vf_loss = pg_loss, vf_loss, neg_entropy, DCV_vf_loss
    ...
    def train(self):
        ...
        print('[*] DCVTD_PG_LOSS:', np.round(pg_loss, 6), '/ DCVTD_VF_LOSS:', np.round(vf_loss, 6), '/ DCVTD_ENT_LOSS:',
              np.round(ent_loss), '/ DCVTD_Value:', np.mean(state_value))

    def save(self, dir_path, step=0):
        model_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, self.name_scope)
        saver = tf.train.Saver(model_vars)

        file_path = os.path.join(dir_path, "DCVTD_ac_{}".format(step))
