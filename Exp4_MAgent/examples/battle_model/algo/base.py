...
class DCV_ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95):
        ...
        with tf.variable_scope(name or "ValueNet"):
            ...
            # DCV Function Input
            self.other_obs_input = self.obs_input
            self.other_feat_input = self.feat_input
            self.other_act_one_hot = self.act_one_hot

            if self.use_mf:
                self.act_prob_input = tf.placeholder(tf.float32, (None, self.num_actions), name="Act-Prob-Input")

            with tf.variable_scope("QNet"):
                self.Q_net_name_scope = tf.get_variable_scope().name
                with tf.variable_scope("Eval-Net"):
                    self.eval_name = tf.get_variable_scope().name
                    self.e_q = self._construct_Q_net(active_func=tf.nn.relu)
                    self.predict = tf.nn.softmax(self.e_q / self.temperature)
                    self.e_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.eval_name)

                with tf.variable_scope("Target-Net"):
                    self.target_name = tf.get_variable_scope().name
                    self.t_q = self._construct_Q_net(active_func=tf.nn.relu)
                    self.t_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.target_name)
            ...
            with tf.variable_scope("DCV-QNet"):
                with tf.variable_scope("DCV-Eval-Net"):
                    self.dcv_eval_name = tf.get_variable_scope().name
                    self.dcv_e_q = self._construct_DCV_Q_net(active_func=tf.nn.relu)
                    self.dcv_predict = tf.nn.softmax(self.dcv_e_q / self.temperature)
                    self.dcv_e_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.dcv_eval_name)

                with tf.variable_scope("DCV-Target-Net"):
                    self.dcv_target_name = tf.get_variable_scope().name
                    self.dcv_t_q = self._construct_DCV_Q_net(active_func=tf.nn.relu)
                    self.dcv_t_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.dcv_target_name)

                with tf.variable_scope("DCV-Update"):
                    self.dcv_update_op = [tf.assign(self.dcv_t_variables[i],
                                                self.tau * self.dcv_e_variables[i] + (1. - self.tau) * self.dcv_t_variables[i])
                                        for i in range(len(self.dcv_t_variables))]

                with tf.variable_scope("DCV-Optimization"):
                    self.dcv_target_q_input = tf.placeholder(tf.float32, (None,), name="DCV-Q-Input")
                    self.dcv_e_q_max = tf.reduce_sum(tf.multiply(self.act_one_hot, self.dcv_e_q), axis=1)
                    self.dcv_loss = tf.reduce_sum(tf.square(self.dcv_target_q_input - self.dcv_e_q_max) * self.mask) / tf.reduce_sum(self.mask)
                    self.dcv_train_op = tf.train.AdamOptimizer(self.lr).minimize(self.dcv_loss)


    def _construct_Q_net(self, active_func=None, reuse=False):
        conv1 = tf.layers.conv2d(self.obs_input, filters=32, kernel_size=3,
                                 activation=active_func, name="Conv1")
        conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=3, activation=active_func,
                                 name="Conv2")
    ...
    def _construct_DCV_Q_net(self, active_func=None, reuse=False):
        conv1 = tf.layers.conv2d(self.obs_input, filters=32, kernel_size=3,
                                 activation=active_func, name="Conv1")
        conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=3, activation=active_func,
                                 name="Conv2")
        ...
        concat_layer = tf.concat([h_obs, h_emb, h_obs, h_emb, self.other_act_one_hot], axis=1)
        if self.use_mf:
            prob_emb = tf.layers.dense(self.act_prob_input, units=64, activation=active_func, name='Prob-Emb')
            h_act_prob = tf.layers.dense(prob_emb, units=32, activation=active_func, name="Dense-Act-Prob")
            concat_layer = tf.concat([concat_layer, h_act_prob], axis=1)
        ...
        dense2 = tf.layers.dense(concat_layer, units=128, activation=active_func, name="Dense2")
        out = tf.layers.dense(dense2, units=64, activation=active_func, name="Dense-Out")
        dcv_q = tf.layers.dense(out, units=self.num_actions, name="DCV-Q-Value")

        return dcv_q

    @property
    def vars(self):
        print(self.Q_net_name_scope)
        return tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.name_scope)

    def calc_intrinsic_reward(self, **kwargs):
        """Calculate the target Q-value
        kwargs: {'obs', 'feature', 'prob', 'dones', 'rewards'}
        """
        feed_dict = {
        ...
        e_q = self.sess.run(self.e_q, feed_dict=feed_dict)
        act_idx = np.argmax(e_q, axis=1)
        q_values = e_q[np.arange(len(e_q)), act_idx]

        dcv_e_q = self.sess.run(self.dcv_e_q, feed_dict=feed_dict)
        dcv_act_idx = np.argmax(dcv_e_q, axis=1)
        dcv_q_values = dcv_e_q[np.arange(len(dcv_e_q)), dcv_act_idx]

        intrinsic_reward = np.mean(q_values - dcv_q_values, axis=0)
        intrinsic_reward = np.clip(0.5*intrinsic_reward, a_min=-0.15, a_max=0.15)
    ...
    def calc_target_q(self, **kwargs):
        ...
        target_q_value = kwargs['rewards'] + kwargs['intrinsic_rewards'] +  (1. - kwargs['dones']) * q_values.reshape(-1) * self.gamma

        return target_q_value

    def calc_dcv_target_q(self, **kwargs):
        """Calculate the target Q-value
        kwargs: {'obs', 'feature', 'prob', 'dones', 'rewards'}
        """
        feed_dict = {
        ...
        e_q = self.sess.run(self.e_q, feed_dict=feed_dict)
        act_idx = np.argmax(e_q, axis=1)
        feed_dict[self.act_input] = act_idx

        dcv_t_q, dcv_e_q = self.sess.run([self.dcv_t_q, self.dcv_e_q], feed_dict=feed_dict)
        dcv_act_idx = np.argmax(dcv_e_q, axis=1)
        dcv_q_values = dcv_t_q[np.arange(len(dcv_t_q)), dcv_act_idx]

        target_dcv_q_value = kwargs['rewards'] + (1. - kwargs['dones']) * dcv_q_values.reshape(-1) * self.gamma

        return target_dcv_q_value

    def update(self):
        """Q-learning update"""
        self.sess.run(self.update_op)
        self.sess.run(self.dcv_update_op)
    ...
    def train(self, **kwargs):
        ...
        feed_dict = {
            ...
            self.feat_input: kwargs['state'][1],
            self.target_q_input: kwargs['target_q'],
            self.dcv_target_q_input: kwargs['dcv_target_q'],
            self.mask: kwargs['masks'],
            self.act_input: kwargs['acts']
        }
        ...
        _, loss, e_q = self.sess.run([self.train_op, self.loss, self.e_q_max], feed_dict=feed_dict)
        _, dcv_loss, dcv_e_q = self.sess.run([self.dcv_train_op, self.dcv_loss, self.dcv_e_q_max], feed_dict=feed_dict)
        return loss, {'DCV-Eval-Q': np.round(np.mean(e_q), 6), 'DCV-Target-Q': np.round(np.mean(kwargs['target_q']), 6)}

class DCVTD_ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95):
        # assert isinstance(env, GridWorld)
        self.env = env
        self.name = name
    ...
    def calc_intrinsic_reward(self, **kwargs):
        ...
        # intrinsic_reward = np.mean(q_values - dcv_q_values, axis=0)
        intrinsic_reward = q_values - dcv_q_values
        intrinsic_reward = np.clip(0.5*intrinsic_reward, a_min=-0.15, a_max=0.15)
