...
from common.ppo_dcv_independent import DCV_PPOPolicyNetwork, DCV_ValueNetwork, DCV_QValueNetwork, DCV_DCQValueNetwork
...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-DCV/" + str(round_num))
    gPi = []
    gV = []
    gQ = []
    gDCQ = []
    ...
    for i in range(n_agent):
        gPi.append(DCV_PPOPolicyNetwork(num_features=env.input_size, num_actions=n_actions, layer_size=256, epsilon=0.1,
                                   learning_rate=lr_actor))
        gV.append(DCV_ValueNetwork(num_features=env.input_size, hidden_size=256, learning_rate=0.001))
        gQ.append(DCV_QValueNetwork(num_features=env.input_size, hidden_size=256, num_actions=n_actions, learning_rate=0.001))
        gDCQ.append(DCV_DCQValueNetwork(num_features=env.input_size, hidden_size=256, num_actions=n_actions, learning_rate=0.001))
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        # ep_intrisic_rewards = [[] for _ in range(n_agent)]
                ...
                for i in range(n_agent):
                    ...
                    for j in range(n_agent):
                        if j != i:
                            ...
                            ep_intrisic_rewards += ep_Q_value - ep_DCQ_value
                    ep_intrisic_rewards = ep_intrisic_rewards/n_agent
                    ep_intrisic_rewards = np.clip(ep_intrisic_rewards, a_min=-0.2, a_max=0.2)
                    if LAMBDA < -0.1:
        ...
        print(config['agent']['env_name'], "PPO-DCV", round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-DCV.npy", reward_step)
