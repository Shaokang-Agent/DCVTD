...
class DCVTD_QValueNetwork():
    def __init__(self, num_features, hidden_size, num_actions, learning_rate=.01):
        ...
        with self.tf_graph.as_default():
            ...
            self.actions = tf.placeholder(shape=[None, self.num_actions], dtype=tf.float32)
            self.W = [
                tf.get_variable("W1", shape=[self.num_features + num_actions, self.hidden_size]),
            ...
            self.inputs = tf.concat([self.observations, self.actions], axis=1)
            self.layer_1 = tf.nn.relu(tf.matmul(self.inputs, self.W[0]) + self.B[0])
    ...
    def get(self, states, actions):
        value = self.session.run(self.output, feed_dict={self.observations: states, self.actions: actions})
        return value

    def update(self, states, actions, discounted_rewards):
        _, loss = self.session.run([self.minimize, self.loss], feed_dict={
            self.observations: states, self.actions: actions, self.rollout: discounted_rewards
        })

class DCVTD_DCQValueNetwork():
    def __init__(self, num_features, hidden_size, num_actions, learning_rate=.01):
        self.num_features = num_features
        self.num_actions = num_actions
        self.hidden_size = hidden_size
        ...
        with self.tf_graph.as_default():
            ...
            self.observations = tf.placeholder(shape=[None, self.num_features], dtype=tf.float32)
            self.other_observations = tf.placeholder(shape=[None, self.num_features], dtype=tf.float32)
            self.actions = tf.placeholder(shape=[None, self.num_actions], dtype=tf.float32)
            self.other_actions = tf.placeholder(shape=[None, self.num_actions], dtype=tf.float32)

            self.W = [
                tf.get_variable("W1", shape=[self.num_features * 2 + 2 * num_actions, self.hidden_size]),
                tf.get_variable("W2", shape=[self.hidden_size, self.hidden_size]),
                tf.get_variable("W3", shape=[self.hidden_size, 1])
            ]
            self.B = [
            ...
            self.inputs = tf.concat([self.observations, self.actions, self.other_observations, self.other_actions], axis=1)
            self.layer_1 = tf.nn.relu(tf.matmul(self.inputs, self.W[0]) + self.B[0])
            self.layer_2 = tf.nn.relu(tf.matmul(self.layer_1, self.W[1]) + self.B[1])
            self.output = tf.reshape(tf.matmul(self.layer_2, self.W[2]) + self.B[2], [-1])
    ...
    def get(self, states, other_states, actions, other_actions):
        value = self.session.run(self.output, feed_dict={self.observations: states,
                                                         self.other_observations: other_states,
                                                         self.actions: actions,
                                                         self.other_actions: other_actions})
        return value

    def update(self, states, other_states, actions, other_actions, discounted_rewards):
        _, loss = self.session.run([self.minimize, self.loss], feed_dict={
...
class Threat_Detection():
    def __init__(self, num_agents, num_features, num_actions, hidden_size, learning_rate=.01):
        self.num_features = num_features
        self.num_actions = num_actions
        self.hidden_size = hidden_size
        self.num_agents = num_agents
        self.head = 4
        self.attention_size = 128 * self.head
        self.tf_graph = tf.Graph()
        with ((self.tf_graph.as_default())):
            self.session = tf.Session()

            self.observations = tf.placeholder(shape=[None, self.num_features], dtype=tf.float32)
            self.other_observations = tf.placeholder(shape=[None, self.num_agents - 1, self.num_features],
                                                     dtype=tf.float32)

            self.value_difference = tf.placeholder(shape=[None, self.num_agents - 1], dtype=tf.float32)
            self.key = tf.layers.dense(inputs=self.observations, units=self.attention_size)
            self.query = tf.layers.dense(inputs=self.other_observations, units=self.attention_size)

            self.key = tf.reshape(self.key, shape=[-1, 1, self.head, self.attention_size // self.head])
            self.key = tf.tile(self.key, multiples=[1, self.num_agents - 1, 1, 1])
            self.key = tf.reshape(self.key, shape=[-1, self.num_agents - 1, self.head, 1, self.attention_size // self.head])
            self.query = tf.reshape(self.query, shape=[-1, self.num_agents - 1, self.head, self.attention_size // self.head, 1])
            att = tf.matmul(self.key, self.query) * (1.0 / tf.sqrt(self.attention_size / self.head))
            att = tf.reshape(att, shape=[-1, self.num_agents - 1, self.head])
            Threat_coefficients = tf.reduce_mean(tf.nn.softmax(att, axis=1), axis=-1)

            self.intrinsic_reward = tf.reduce_mean(Threat_coefficients * self.value_difference, axis=-1)

            self.rollout = tf.placeholder(shape=[None], dtype=tf.float32)
            self.V_value = tf.placeholder(shape=[None], dtype=tf.float32)
            self.loss = - tf.reduce_mean((self.rollout-self.V_value)*self.intrinsic_reward)
            self.grad_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
            self.minimize = self.grad_optimizer.minimize(self.loss)
    ...
    def get(self, observations, other_observations, value_difference):
        intrinsic_reward = self.session.run(self.intrinsic_reward, feed_dict={
            self.observations: observations,
            self.other_observations: other_observations,
            self.value_difference: value_difference})
        return intrinsic_reward

    def update(self, observations, other_observations, value_difference, initial_return, v_value):
        _, loss = self.session.run([self.minimize, self.loss], feed_dict={
            self.observations: observations,
            self.other_observations: other_observations,
            self.value_difference: value_difference,
            self.rollout: initial_return,
            self.V_value: v_value
        })

class DCVTD_ValueNetwork():
